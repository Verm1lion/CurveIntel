"""
CurveIntel vendor CSV profiles and automatic detection helpers.
...
Supported exports:
  ...
  - DEVOTRANS CKS-III (CSV, Turkish locale)
  - Hegewald & Peschke LabMaster (CSV, German locale)
  - NIST Numisheet (CSV)
  - Generic fallback
"""
...
@dataclass
class VendorProfile:
    """Machine/software CSV export profile."""
...
ZWICKROELL = VendorProfile(
    ...
    column_map={
        ...
        "F": "force",
        # Displacement
        "Standardweg": "displacement",
        "Standard travel": "displacement",
        "Traverse": "displacement",
        "Traversenweg": "displacement",
        "Weg": "displacement",
    ...
    default_separator=";",
    time_column="Zeit",
    notes="DE locale uses ';' as the separator and ',' as the decimal mark. EN locale flips both.",
)
...
INSTRON = VendorProfile(
    ...
    column_map={
        ...
        # Displacement
        "Extension": "displacement",
        "Extension (mm)": "displacement",
        "Compressive extension": "displacement",
        "Crosshead": "displacement",
        "Weg": "displacement",
        "Allongement": "displacement",
        "Uzama": "displacement",
        # Stress
        "Tensile stress": "stress",
        "Tensile stress (MPa)": "stress",
        "Compressive stress": "stress",
        "Flexure stress": "stress",
        "Stress": "stress",
        "Zugspannung": "stress",
        "Contrainte": "stress",
        "Gerilme": "stress",
    ...
    default_separator=",",
    time_column="Time",
    notes="UTF-16 BOM variants exist, so encoding detection must stay enabled.",
)
...
SHIMADZU = VendorProfile(
    ...
    default_separator=",",
    time_column="\u6642\u9593",
    notes="Shift-JIS is common for Japanese exports. Western variants can be ASCII or CP1252.",
)
...
DEVOTRANS = VendorProfile(
    ...
    default_separator=";",
    time_column="Zaman",
    notes="Turkish locale uses ';' as the separator and ',' as the decimal mark.",
)
...
# Precedence order: specific -> general
...
ENCODING_CHAIN = [
    ...
    "windows-1254",  # DEVOTRANS (Turkish locale)
    "latin-1",  # Last-resort fallback
]


def detect_vendor(filepath, max_lines: int = 40) -> VendorProfile | None:
    """
    Detect the vendor profile for a CSV export.

    Two-stage approach:
      1. Read the first N lines and score fingerprint regex matches.
      2. Return the best-scoring profile from ALL_PROFILES.

    Args:
        filepath: CSV file path
        max_lines: Maximum number of lines to inspect

    Returns:
        A matched VendorProfile, or None when no profile wins
    """
    from pathlib import Path

    filepath = Path(filepath)

    # Read the header block using the encoding fallback chain.
    raw_text = ""
    ...
    if not raw_text:
        # Last resort: latin-1 always decodes.
        with open(filepath, "r", encoding="latin-1") as f:
            lines = [f.readline() for _ in range(max_lines)]
        raw_text = "\n".join(lines)

    # Fingerprint match: choose the profile with the highest match count.
    best_profile = None
    best_score = 0
...
def detect_encoding(filepath) -> str:
    """
    Detect the text encoding for a file.

    Uses BOM checks first, then falls back to the encoding chain.
    """
    from pathlib import Path
    ...
    # Try the configured encoding chain.
    for enc in ENCODING_CHAIN:
        try:
            with open(filepath, "r", encoding=enc, errors="strict") as f:
                f.read(4096)
...
def detect_decimal_separator(sample_lines: list[str], separator: str = ",") -> str:
    """
    Detect the decimal separator used by numeric cells.

    Compare the number of commas and dots inside cells that look numeric.
    """
    dot_count = 0
    comma_count = 0
    ...
    for line in sample_lines:
        ...
        for cell in cells:
            ...
            # Does the cell look numeric?
            if re.match(r"^-?\d", cell):
                dot_count += cell.count(".")
                comma_count += cell.count(",")

    # Prefer the dominant decimal symbol when commas clearly outnumber dots.
