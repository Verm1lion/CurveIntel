...
def run_full_pipeline(csv_path: Path, label: str):
    ...
    for r in ctx.step_results:
        ...
        if r.status.value == "success": ok += 1
        elif r.status.value == "warning": warn += 1
        else: fail += 1
    ...
    pairs = [
        ...
        ("Yield", f"{p.yield_strength_mpa:.1f} MPa ({p.yield_behavior.value})" if p.yield_strength_mpa else "---"),
        ...
        ("Uniform Elong.", f"{p.uniform_elongation_pct:.2f}%" if p.uniform_elongation_pct else "---"),
    ...
    try:
        ...
        if ys and p.yield_strength_mpa:
            ax.plot(ys, p.yield_strength_mpa, "go", ms=10,
                    label=f"Yield={p.yield_strength_mpa:.0f} MPa")

        # UTS
        uts_idx = ctx.extra.get("uts_idx")
        if uts_idx and p.ultimate_tensile_mpa:
            ax.plot(ctx.strain[uts_idx], p.ultimate_tensile_mpa, "r^", ms=10,
...
def main():
    ...
    nist_csv = base / "nist_numisheet" / "C00Al6xxxT4Numisheet2020R01T1.521W17.91-S-Stress-Strain.csv"
    if nist_csv.exists():
        run_full_pipeline(nist_csv, "NIST Al6xxx-T4")

    # Test 2: Zenodo S355J2
    zenodo_csv = (base / "Zenodo Structural Metallic DB" / "Clean_Data_v1-0-0"
                  / "Clean_Data" / "S355J2_Plates" / "S355J2_N_25mm" / "S_8_00_N_5.csv")
